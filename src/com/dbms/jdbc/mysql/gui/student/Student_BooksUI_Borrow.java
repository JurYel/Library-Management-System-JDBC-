...
public class Student_BooksUI_Borrow extends javax.swing.JDialog {

    /**
     * Creates new form Student_BooksUI_Borrow
     */
    private static final DateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

    private Student_BooksUI studUI;
    private String UserID;
    private BooksDAO bookDAO;
    private IssuedBooksDAO issuedDAO;
    private Books book;

    public Student_BooksUI_Borrow(String userID, Books theBook, BooksDAO dao, Student_BooksUI ui) {
        try {

            issuedDAO = new IssuedBooksDAO();
        } catch (Exception exc) {
            JOptionPane.showMessageDialog(Student_BooksUI_Borrow.this, "DAO Failed: " + exc.getMessage(), "DAO Error", JOptionPane.ERROR_MESSAGE);
        }
        initComponents();
    ...
    public Student_BooksUI_Borrow(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Student_BooksUI_Borrow.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                Student_BooksUI_Borrow dialog = new Student_BooksUI_Borrow(new javax.swing.JFrame(), true);
    ...
    private void borrowBook() {
        ...
        try {
            if (issuedDAO.checkIfAlreadyBorrowed(userID, id)) {
                ...
                if (total <= 5) {
                    issuedDAO.IssueBook(issuedBook, UserID, quantity);

                    setVisible(false);
                    studUI.refreshBooksTable();
                    JOptionPane.showMessageDialog(studUI,
                    ...
                    setVisible(false);
                    studUI.refreshBooksTable();
                    JOptionPane.showMessageDialog(studUI, "You can only borrow five books at most!", "Issue Error", JOptionPane.ERROR_MESSAGE);
                }
            }
        } catch (Exception exc) {
            JOptionPane.showMessageDialog(Student_BooksUI_Borrow.this, "Issue Failed: " + exc.getMessage(), "Issue Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void retrieveIssuedBook() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(Student_BooksUI_Borrow.this, "Retrieval Failed: " + exc.getMessage(), "Retrieval Error", JOptionPane.ERROR_MESSAGE);
//            exc.printStackTrace();
        }
    }

    private void populateGUI(Books theBook) {
        jComboBox_Users.addItem(UserID);
        jComboBox_Users.setSelectedItem(UserID);
        jComboBox_Category.setSelectedItem(theBook.getBookCategory());
        jtxt_BookID.setText(theBook.getBookID());
        jtxt_Title.setText(theBook.getBookTitle());
        jtxt_Author.setText(theBook.getBookAuthor());
        jSpinner_Rack.setValue(theBook.getRack());
        jtxt_Year.setText(String.valueOf(theBook.getYearPublished()));
        jSpinner_Quantity.setValue(theBook.getQuantity());
    }
